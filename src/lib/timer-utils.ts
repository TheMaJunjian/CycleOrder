...
export function formatTime(milliseconds: number, showMilliseconds: boolean = false): string {
  ...
  if (days > 0) {
    return `${days}d ${hours % 24}:${String(minutes % 60).padStart(2, '0')}:${String(seconds % 60).padStart(2, '0')}${showMilliseconds ? `.${String(ms).padStart(2, '0')}` : ''}`
  }
  if (hours > 0) {
    return `${hours}:${String(minutes % 60).padStart(2, '0')}:${String(seconds % 60).padStart(2, '0')}${showMilliseconds ? `.${String(ms).padStart(2, '0')}` : ''}`
  }
  return `${String(minutes).padStart(2, '0')}:${String(seconds % 60).padStart(2, '0')}${showMilliseconds ? `.${String(ms).padStart(2, '0')}` : ''}`
}
